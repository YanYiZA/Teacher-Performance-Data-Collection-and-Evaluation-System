...
CTeacherPerformanceDataCollectionandEvaluationSystemDlg::CTeacherPerformanceDataCollectionandEvaluationSystemDlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_TEACHERPERFORMANCEDATACOLLECTIONANDEVALUATIONSYSTEM_DIALOG, pParent),m_pImport(nullptr),m_pRankingManager(nullptr)
{
...
BEGIN_MESSAGE_MAP(CTeacherPerformanceDataCollectionandEvaluationSystemDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON_EXPORT_WORD, &CTeacherPerformanceDataCollectionandEvaluationSystemDlg::OnBnClickedButtonExportWord)
END_MESSAGE_MAP()
...
BOOL CTeacherPerformanceDataCollectionandEvaluationSystemDlg::OnInitDialog()
{
	...
	// 初始化 CImport 对象并与 CListCtrl 关联
	m_pImport = new CImport(&m_listData);
...
void CTeacherPerformanceDataCollectionandEvaluationSystemDlg::OnBnClickedButtonAdd()
{
	...
	// 添加到导入的数据列表中
	TeacherPerformance newPerformance;
	newPerformance.name = name;
	newPerformance.teachingPerformance = teaching;
	newPerformance.teachPerformance = teach;
	newPerformance.researchPerformance = research;
	newPerformance.otherPerformance = other;
	newPerformance.totalPerformance = total;
	newPerformance.rank = 0;
	// 添加到 m_dataList
	m_dataList.push_back(newPerformance);

	// 更新排名
	m_pRankingManager->UpdateRanking(m_dataList);
...
void CTeacherPerformanceDataCollectionandEvaluationSystemDlg::UpdateListCtrl()
{
	...
	for (size_t i = 0; i < m_dataList.size(); ++i)
	{
		...
		// 添加绩效数据
		m_listData.SetItemText(index, 1, CString(std::to_wstring(data.teachingPerformance).c_str()));
		m_listData.SetItemText(index, 2, CString(std::to_wstring(data.teachPerformance).c_str()));
		m_listData.SetItemText(index, 3, CString(std::to_wstring(data.researchPerformance).c_str()));
		m_listData.SetItemText(index, 4, CString(std::to_wstring(data.otherPerformance).c_str()));
		m_listData.SetItemText(index, 5, CString(std::to_wstring(data.totalPerformance).c_str()));
		m_listData.SetItemText(index, 6, CString(std::to_wstring(data.rank).c_str()));
	}
}
...
void CTeacherPerformanceDataCollectionandEvaluationSystemDlg::OnBnClickedButtonImport()
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		// 使用指针创建 CImport 对象，直接操作 m_pListCtrl
		CImport import(&m_listData);
        import.ImportDocxFiles(filePaths);

		// 将数据同步到 m_dataList
		int itemCount = m_listData.GetItemCount();
		m_dataList.clear();
		for (int i = 0; i < itemCount; i++)
		{
			TeacherPerformance data;
			data.name = m_listData.GetItemText(i, 0);
			data.teachingPerformance = _ttoi(m_listData.GetItemText(i, 1));
			data.teachPerformance = _ttoi(m_listData.GetItemText(i, 2));
			data.researchPerformance = _ttoi(m_listData.GetItemText(i, 3));
			data.otherPerformance = _ttoi(m_listData.GetItemText(i, 4));
			data.totalPerformance = data.teachingPerformance + data.teachPerformance + data.researchPerformance + data.otherPerformance;
			data.rank = 0;  // 初始排名设为 0
...
void CTeacherPerformanceDataCollectionandEvaluationSystemDlg::ShowErrorMessage(const CString& message)
{
	MessageBox(message, _T("错误"), MB_ICONERROR);
}
...
void CTeacherPerformanceDataCollectionandEvaluationSystemDlg::OnBnClickedButtonExportWord()
{
	...
	if (m_dataList.empty())
	{
		MessageBox(_T("没有数据可供导出！"), _T("提示"), MB_ICONINFORMATION);
		return;
	}

	// 保存文件对话框
	CFileDialog fileDlg(FALSE, _T("docx"), _T("教师绩效报告.docx"),
		OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
		_T("Word 文件 (*.docx)|*.docx|所有文件 (*.*)|*.*||"));
	...
	if (fileDlg.DoModal() == IDOK)
	{
		...
		// 创建Word实例
		CMSWord* pWord = new CMSWord();
		if (!pWord->CreateWordInstance())
		{
			MessageBox(_T("无法创建Word实例！"), _T("错误"), MB_ICONERROR);
			delete pWord;
			return;
		}

		// 创建新文档
		if (!pWord->CreateNewDocument())
		{
			MessageBox(_T("无法创建新文档！"), _T("错误"), MB_ICONERROR);
			delete pWord;
			return;
		}

		// 检查关键对象是否正确初始化
		if (!pWord->m_pSelection)
		{
			MessageBox(_T("m_pSelection 未初始化！"), _T("错误"), MB_ICONERROR);
			delete pWord;
			return;
		}

		if (!pWord->m_pDocument)
		{
			MessageBox(_T("文档未正确初始化！"), _T("错误"), MB_ICONERROR);
			delete pWord;
			return;
		}

		// 添加标题
		pWord->AddTitle(_T("教师绩效评估报告"));
		pWord->AddParagraph(_T("")); // 空行

		// 添加生成时间
		CTime currentTime = CTime::GetCurrentTime();
		CString timeStr = currentTime.Format(_T("生成时间：%Y年%m月%d日"));
		pWord->AddParagraph(timeStr);
		pWord->AddParagraph(_T("")); // 空行

		// 添加表格
		int rowCount = m_dataList.size() + 1; // 数据行 + 表头
		int colCount = 6;                    // 6列

		if (pWord->CreateTable(rowCount, colCount))
		{
			// 设置表头
			pWord->SetTableCell(1, 1, _T("姓名"));
			pWord->SetTableCell(1, 2, _T("教学工作"));
			pWord->SetTableCell(1, 3, _T("教研工作"));
			pWord->SetTableCell(1, 4, _T("科研工作"));
			pWord->SetTableCell(1, 5, _T("其他工作"));
			pWord->SetTableCell(1, 6, _T("总绩效"));
			...
			for (const auto& data : m_dataList)
			{
				...
				// 姓名
				if (!pWord->SetTableCell(row, 1, data.name)) {
					AfxMessageBox(_T("设置姓名失败"));
					break;
				}

				// 教学工作
				formattedValue.Format(_T("%.2f"), data.teachingPerformance);
				if (!pWord->SetTableCell(row, 2, formattedValue)) {
					AfxMessageBox(_T("设置教学工作失败"));
					break;
				}

				// 教研工作
				formattedValue.Format(_T("%.2f"), data.teachPerformance);
				if (!pWord->SetTableCell(row, 3, formattedValue)) {
					AfxMessageBox(_T("设置教研工作失败"));
					break;
				}

				// 科研工作
				formattedValue.Format(_T("%.2f"), data.researchPerformance);
				if (!pWord->SetTableCell(row, 4, formattedValue)) {
					AfxMessageBox(_T("设置科研工作失败"));

				// 其他工作
				formattedValue.Format(_T("%.2f"), data.otherPerformance);
				if (!pWord->SetTableCell(row, 5, formattedValue)) {
					AfxMessageBox(_T("设置其他工作失败"));
					break;
				}

				// 总绩效
				formattedValue.Format(_T("%.2f"), data.totalPerformance);
				if (!pWord->SetTableCell(row, 6, formattedValue)) {
					AfxMessageBox(_T("设置总绩效失败"));
					break;
				}
				...
				AfxMessageBox(_T("创建表格成功！"));
			}
		}
		else
		{
			MessageBox(_T("创建表格失败！"), _T("错误"), MB_ICONERROR);
		}

		// 保存并关闭文档
		if (!pWord->SaveAs(filePath))
		{
			MessageBox(_T("保存文档失败！"), _T("错误"), MB_ICONERROR);
		}
